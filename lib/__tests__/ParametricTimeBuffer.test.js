...
describe('scitylana::ParametricTimeBuffer', function() {
  const sinon = createSinonSuite(this);
  const { allows, step } = ParametricTimeBuffer;
  const createStrategy = ParametricTimeBuffer;
  ...
  context('when the event with that data has not been tracked yet...', function() {
    it('is ok', function() {
      assert.ok(allows(base, event, data1))
    })
  })
  ...
  context('when the event has been tracked...', function() {
    ...
    it('is ok if the data differs', function() {
      assert.ok(
        allows(withData1, event, data2)
      )
    });

    it('is false otherwise', function() {
      assert.notOk(allows(withData1, event, data1))
    });
  })
  ...
  context('when the event has been tracked and the period has elapsed...', function() {
    it('is ok', function() {
      ...
      assert.ok(allows(withEvent, event, data1))
    })
  })
  ...
  it('maintains a timer for each bucket', function() {
    ...
    assert.ok(allows(withFirstEvent, event, data2));

    const withSecondEvent = step(withFirstEvent, event, data2);

    sinon.clock.tick(Period.toMilliseconds(base.period) + 1 - 1000)

    assert.ok(allows(withSecondEvent, event, data1));
    assert.notOk(allows(withSecondEvent, event, data2));
  })
})