...
describe('scitylana::Adapter', function() {
  ...
  describe('.advance', function() {
    ...
    it('steps event sequences (EventSequence.Any)', function() {
      const eventA = { name: 'a' }
      const eventB = { name: 'b' }
      const metric = { events: EventSequence.Any([ eventA.name, eventB.name ]) }
      const initial = Adapter.create([ metric ])

      assert.equal(Adapter.reduce(initial, [ eventA ]).length, 1)
      assert.equal(Adapter.reduce(initial, [ eventB ]).length, 1)
      assert.equal(Adapter.reduce(initial, [ eventA, eventB ]).length, 2)

      const advanced = Adapter.advance(initial, Adapter.reduce(initial, [ eventA ]));

      assert.equal(Adapter.reduce(advanced, [ eventA ]).length, 1)
      assert.equal(Adapter.reduce(advanced, [ eventB ]).length, 1)
      assert.equal(Adapter.reduce(advanced, [ eventA, eventB ]).length, 2)
    })

    it('steps event sequences (EventSequence.Ordered)', function() {
      const eventA = { name: 'a' }
      const eventB = { name: 'b' }
      const metric = { events: EventSequence.Ordered([ eventA.name, eventB.name ]) }
      const initial = Adapter.create([ metric ])

      const forA = Adapter.reduce(initial, [ eventA ])

      assert.equal(forA.length, 1)

      const afterA = Adapter.advance(initial, forA);

      assert.equal(Adapter.reduce(afterA, [ eventA ]).length, 0);

      const forB = Adapter.reduce(afterA, [ eventB ]);

      assert.equal(forB.length, 1);

      const afterB = Adapter.advance(afterA, forB);

      assert.equal(Adapter.reduce(afterB, [ eventA ]).length, 1)
      assert.equal(Adapter.reduce(afterB, [ eventB ]).length, 0)
    })
  })
})